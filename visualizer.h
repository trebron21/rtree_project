...
//    double w = 10 + ( rand() % 1000 ) / 100.0;
//    double h = 10 + ( rand() % 1000 ) / 100.0;
    ...
    {
      double theta = i * 360 / double(nofPoints);
      double xc = x + r * cos(theta * pi() / 180.);
      double yc = y + r * sin(theta * pi() / 180.);
      search_ring.push_back(P(xc, yc));
    } // todo: the ring is not a closed line, the last point is not connected to the first
...
void search()
{
    ...
    namespace dp = d::predicates;

    if ( query_mode == qm_knn || query_mode == qm_knnb || query_mode == qm_knns )
        query_knn();
    else if ( query_mode == qm_d )
        query< dp::spatial_predicate<B, dp::disjoint_tag, false> >();
    else if ( query_mode == qm_i )
        query< dp::spatial_predicate<B, dp::intersects_tag, false> >();
    else if ( query_mode == qm_nd )
        query< dp::spatial_predicate<B, dp::disjoint_tag, true> >();
    else if ( query_mode == qm_ni )
        query< dp::spatial_predicate<B, dp::intersects_tag, true> >();
...
//        query< dp::spatial_predicate<B, dp::covered_by_tag, false> >();
//    else if ( query_mode == qm_o )
//        query< dp::spatial_predicate<B, dp::overlaps_tag, false> >();
//    else if ( query_mode == qm_w )
//        query< dp::spatial_predicate<B, dp::within_tag, false> >();
//    else if ( query_mode == qm_nc )
//        query< dp::spatial_predicate<B, dp::covered_by_tag, true> >();
//    else if ( query_mode == qm_no )
//        query< dp::spatial_predicate<B, dp::overlaps_tag, true> >();
//    else if ( query_mode == qm_nw )
//        query< dp::spatial_predicate<B, dp::within_tag, true> >();
    else if ( query_mode == qm_ri )
        query_ring< dp::spatial_predicate<R, dp::intersects_tag, false> >();
//    else if ( query_mode == qm_pi )
//        query_poly< dp::spatial_predicate<Poly, dp::intersects_tag, false> >();
//    else if ( query_mode == qm_mpi )
//        query_multi_poly< dp::spatial_predicate<MPoly, dp::intersects_tag, false> >();
//    else if ( query_mode == qm_si )
//        query_segment< dp::spatial_predicate<S, dp::intersects_tag, false> >();
//    else if ( query_mode == qm_lsi )
//        query_linestring< dp::spatial_predicate<LS, dp::intersects_tag, false> >();
